app.controller('apunController', function($rootScope, $scope, $http, $routeParams, $q, srv) {
	...
	function srvApunSave() {
		...
		if ($scope.apun.fval === undefined || $scope.apun.fval === '' || $scope.apun.fval === null) {
			...
			//FIXME: clean y revisar compatibilidad del objeto date
			var fval = $scope.apun.fval.toString();
			fval = fval.replace('/', '-');
			fval = fval.replace('.', '-');
			fval = fval.replace('.', '-');
			//$scope.apun.fval = fval;
		...
		var dataObject = {
			...
			fval : parseInt(formatFval),
			adic : $scope.apun.adic,
